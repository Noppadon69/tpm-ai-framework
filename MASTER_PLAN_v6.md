...
>
> **เวอร์ชัน:** 6.0 | **วันที่:** 9 พฤษภาคม 2026 | **สถานะ:** พร้อมเริ่ม Phase 0
> **เปลี่ยนจาก v5.0:** แก้ช่องโหว่ G-01–G-15 จาก Gap Analysis รอบ 1 + เพิ่ม Mold & Die Domain (Section 25)
...
## 1.2b อะไรเปลี่ยนจาก v5.0 → v6.0

| ประเด็น | v5.0 | v6.0 |
|---|---|---|
| Layer 1 stack | OpenKB + PageIndex (ไม่มีบน PyPI) | **markitdown + llama-index (verified)** |
| Vision VRAM | 3 options ไม่มี default | **Default: Option C — Qwen2.5-VL-3B** |
| Phase 0 setup | ไม่มี Docker | **เพิ่ม Docker + prevent_sleep + .gitignore** |
| Night Cycle | ไม่ป้องกัน Windows sleep | **prevent_sleep.ps1 + Task Scheduler** |
| GitHub backup | models/ ไม่มี exclude | **models/ ใน .gitignore + MODEL_LIST.txt** |
| Dependencies | ไม่มี version pin | **requirements.txt + requirements.lock** |
| Mold Domain | Section 25 ไม่ link Phase | **golden_dataset M1-M4 + acceptance criteria** |
| Activity Tier 3 | active-win-listener WSL2 พัง | **PowerShell Get-Process fallback** |
| Mem0 | ไม่ระบุ dependency | **ChromaDB collection แทน** |
| SearXNG engines | ไม่จำกัด → ถูก block | **จำกัด safe engines + rate limit** |
| Coder model | qwen3-coder (ชื่อไม่แน่) | **verify ก่อน pull / fallback qwen2.5-coder** |
| OpenFOAM | Phase 5 optional | **Defer ออก — เกิน scope intern** |
| Path | ปน Windows/WSL2 | **canonical: /mnt/d/tpm_workspace** |
...
│  ✅ v6.0 DEFAULT = Option C:                                  │
│    C) Qwen2.5-VL-3B (~2 GB) ← DEFAULT — VRAM OK (~8 GB)    │
│    Fallback B) CPU+GPU split 7B ถ้า accuracy ไม่พอ          │
│    ❌ ห้าม A — unload orchestrator ทำ state loss            │
└─────────────────────────────────────────────────────────────┘
...
```
/mnt/d/tpm_workspace/                   # canonical path (WSL2) — ดู G-14
# Windows native: D:\tpm_workspace | Python code ใช้ Path("/mnt/d/tpm_workspace")
│
...
```bash
# OpenKB
# markitdown (Microsoft MIT) — ทดแทน openkb
pip install markitdown

# llama-index — document ingestion + query (ทดแทน pageindex)
pip install llama-index llama-index-embeddings-ollama llama-index-vector-stores-chroma
...
2. รัน `markitdown <file>` แปลง PDF/Word/Excel → Markdown ใน wiki/sources/
3. llama-index VectorStoreIndex สร้าง index บน ChromaDB
4. Query ผ่าน llama-index QueryEngine (semantic + keyword hybrid)
5. Incremental update: เพิ่มเอกสาร → index เฉพาะไฟล์ใหม่
...
day_1_morning:
  ...
  - [ ] ติดตั้ง Docker Desktop for Windows + enable WSL2 backend
  - [ ] ทดสอบ: docker run hello-world && docker run --gpus all nvidia/cuda:12.0-base nvidia-smi
  - [ ] สร้าง .gitignore (ต้องมี models/, chroma_db/, .venv/, logs/, __pycache__/)
  - [ ] สร้าง models/MODEL_LIST.txt (บันทึกชื่อ model — แทน backup ไฟล์จริง)
  - [ ] สร้าง Python venv (Python 3.11+)
  - [ ] สร้าง requirements.txt พร้อม version pin แล้วติดตั้ง:
      pip install -r requirements.txt
      pip freeze > requirements.lock
  - [ ] สร้าง scripts/prevent_sleep.ps1 (ป้องกัน Windows sleep ช่วง night cycle)
  - [ ] ตั้ง Task Scheduler: 22:55 → prevent_sleep.ps1 / 05:05 → restore sleep
...
must_pass:
  ...
  - docker run hello-world ผ่าน + GPU visible ใน container
  - เครื่องไม่ sleep หลัง 1 ชม. เมื่อ prevent_sleep.ps1 รันอยู่
  - Restore drill: สร้าง test file → commit → ลบ → pull กลับมาได้
  - .gitignore exclude models/ และ chroma_db/ (ทดสอบด้วย git status)
```
...
day_1:
  ...
  - [ ] ติดตั้ง markitdown + llama-index (ดู G-01 patch)
  - [ ] รัน markitdown แปลงทุกไฟล์ใน raw_data/ → .tpm_context/wiki/sources/ (30-60 นาที)
  - [ ] สร้าง llama-index VectorStoreIndex บน ChromaDB (incremental ทำใหม่ได้)
...
day_3:
  - [ ] เขียน Inquiry-First (Section 8):
      ...
      - ask_user_first
      - User Memory: ChromaDB collection "user_memory" (แทน Mem0 — ดู G-09)
...
day_3:
  - [ ] ดาวน์โหลด Coder model (verify ก่อน — ดู G-11):
      ollama search qwen3-coder  # ตรวจว่ามีใน registry
      ollama pull qwen2.5-coder:7b-q4_K_M  # verified fallback
...
day_5:
  ...
  - [ ] เพิ่ม golden_dataset mold cases M1-M4 (ดู Section 25 + G-07)
  - [ ] golden_dataset retest → 24/30 ผ่าน (รวม mold cases ≥ 3/4)
```
...
must_pass:
  ...
  - golden_dataset 8/10 general cases ผ่าน end-to-end + auditor
  - golden_dataset mold cases ≥ 3/4 ผ่าน (MoldAnalyseNode)
  - Shot counter alert ทำงานเมื่อ shots เกิน threshold
```
...
day_2:
  - [ ] เขียน activity_tracker.py (Section 14):
      ...
      - Tier 3: Outside-AI tracking ผ่าน PowerShell Get-Process (ดู G-08)
        # active-win-listener ไม่รองรับ WSL2 — ใช้ subprocess → powershell.exe แทน
...
| R16 | markitdown ไม่รองรับ format บางอย่าง | 🟡 Medium | 🟡 Medium | camelot-py + tabula-py สำรองสำหรับ PDF table | Phase 1 |
| R17 | SearXNG IP ถูก block | 🟠 High | 🟠 High | ใช้เฉพาะ safe engines + Tavily fallback (ดู G-10) | L3 router |
| R18 | Windows sleep ระหว่าง Night Cycle | 🔴 High | 🟡 Medium | prevent_sleep.ps1 + Task Scheduler (ดู G-04) | prevent_sleep |
...
**END OF MASTER PLAN v6.0**

> **Last updated:** 9 พฤษภาคม 2026
> **Total sections:** 26 (เพิ่ม Mold & Die Domain + Gap Analysis)
> **Total length:** ~5,700+ บรรทัด
> **License:** MIT (ผู้ใช้ฟรี + commercial OK)
> **Maintainer:** TPM Intern + AI co-pilot
> **Gap status:** G-01–G-15 patched ใน v6.0



---

# 25. Mold & Die Domain Extension (Toshiba Internship — เครื่องซักผ้า)

> **เพิ่มเมื่อ:** 9 พฤษภาคม 2026
> **เหตุผล:** แผนฝึกงาน Toshiba (ฝ่ายแม่พิมพ์เครื่องซักผ้า) ครอบคลุมทักษะที่ MASTER_PLAN_v5.0 ยังไม่ได้ระบุ
> **ขอบเขต:** Injection Mold + Press Die สำหรับชิ้นส่วนเครื่องซักผ้า

---

## 25.1 Gap Analysis — เทียบแผนฝึกงาน vs MASTER_PLAN_v5.0

| หัวข้อจากแผนฝึกงาน | ครอบคลุมใน v5.0? | ช่องว่างที่ต้องเพิ่ม |
|---|---|---|
| โครงสร้างแม่พิมพ์ (Core/Cavity/Slide/Ejector) | ❌ ไม่มี | ต้องสร้าง Knowledge domain เฉพาะ Mold & Die |
| วัสดุแม่พิมพ์ (SKD11, SKD61, S50C, P20, NAK80) | ❌ ไม่มี | ต้องเพิ่ม Material property database |
| อ่านแบบ 2D/3D + วัดชิ้นงาน | ⚠️ มีแค่ OCR PDF ทั่วไป | ต้องเพิ่ม DXF/STEP parser + Tolerance checker |
| PM Check Sheet (shot counter, maintenance log) | ⚠️ มีแค่ Excel ทั่วไป | ต้องทำ Mold-specific PM tracker |
| Defect analysis (Flash, Sink mark, Warpage, Burr) | ⚠️ มีแค่ CrackNet (ผนังปูน) | ต้องเพิ่ม Mold defect classifier |
| Process parameter (อุณหภูมิ, ความดัน, cycle time) | ❌ ไม่มี | ต้องเพิ่ม Process log reader |
| DFM / Root cause analysis | ❌ ไม่มี | ต้องเพิ่ม Mold analyse workflow ใน LangGraph |
| Safety & SOP | ⚠️ มีแค่ Human Gate | ต้องเพิ่ม SOP checklist สำหรับงานแม่พิมพ์โดยเฉพาะ |

---

## 25.2 สิ่งที่ควรเพิ่มในแผน (นอกเหนือจากข้อมูลเดิม)

### 25.2.1 Process Parameters — จุดบอดที่ใหญ่ที่สุด

แผนเดิมเน้น "ซ่อมแม่พิมพ์" แต่ถ้าไม่รู้ค่า process จะ root cause ไม่ได้ ควรเพิ่ม:

```yaml
injection_mold_params:
  - barrel_temperature     # อุณหภูมิกระบอกฉีด (°C) แต่ละโซน
  - mold_temperature       # อุณหภูมิแม่พิมพ์ (°C)
  - injection_pressure     # ความดันฉีด (MPa)
  - holding_pressure       # ความดันอัด (MPa)
  - cooling_time           # เวลาเย็นตัว (s)
  - cycle_time             # เวลารวมต่อ shot (s)

press_die_params:
  - tonnage                # แรงปั๊ม (ton)
  - stroke                 # ระยะชัก (mm)
  - feed_rate              # ความเร็วป้อนแผ่น (mm/stroke)
  - material_thickness     # ความหนาวัสดุ (mm)
```

**ทำไมสำคัญ:** ถ้า Sink mark เกิดขึ้น สาเหตุอาจเป็น holding_pressure ต่ำ หรือ cooling_time สั้น ไม่ใช่แม่พิมพ์พัง — วิเคราะห์ผิดทิศถ้าไม่มีข้อมูลนี้

### 25.2.2 Defect Catalogue สำหรับแม่พิมพ์โดยเฉพาะ

```yaml
injection_mold_defects:
  Flash:        พลาสติกล้นแนวแยกแม่พิมพ์ → Clamping force ต่ำ / Parting surface สึก
  Sink_mark:    รอยบุ๋มผิว → Holding pressure ต่ำ / Gate เล็ก / ผนังหนาเกิน
  Short_shot:   ฉีดไม่เต็ม → อุณหภูมิต่ำ / Injection pressure ต่ำ / Vent อุดตัน
  Warpage:      ชิ้นงานโก่ง → ระบบน้ำเย็นไม่สมดุล / Ejection ไม่สม่ำเสมอ
  Burn_mark:    รอยไหม้ → ติดก๊าซ (Vent ไม่พอ) / อุณหภูมิสูงเกิน
  Weld_line:    รอยประสาน → น้ำพลาสติกสองทางมาบรรจบกัน

press_die_defects:
  Burr:         ขอบคม → Clearance มากเกิน / Punch/Die สึกหรอ
  Springback:   ชิ้นงานคืนรูป → Overbend ชดเชยไม่ถูก
  Crack:        วัสดุแตก → R มุมน้อยเกิน / แรงปั๊มมากเกิน
  Wrinkle:      ผิวยับ → Blank holder force ต่ำเกิน
  Galling:      ผิวขูด → Lubrication ไม่พอ / Clearance น้อยเกิน
```

### 25.2.3 Mold Life & Shot Counter — สิ่งที่ขาดไปจากแผนเดิม

```python
# ตัวอย่าง logic ที่ควรมีใน PM tracker
mold_life_rules = {
    "Injection_Mold_Steel": {
        "SKD61": {"PM_interval": 50_000,  "overhaul": 500_000},   # shots
        "P20":   {"PM_interval": 30_000,  "overhaul": 300_000},
        "NAK80": {"PM_interval": 20_000,  "overhaul": 200_000},
    },
    "Press_Die_Steel": {
        "SKD11": {"PM_interval": 100_000, "overhaul": 1_000_000}, # strokes
        "S50C":  {"PM_interval": 10_000,  "overhaul": 100_000},
    }
}
```

### 25.2.4 Reporting & Communication Skills — ที่แผนเดิมขาด

ควรฝึกเขียน:
- **Maintenance Report** — ทำอะไร, เปลี่ยนอะไร, ผลหลังซ่อม
- **NCR (Non-Conformance Report)** — บันทึก defect อย่างเป็นทางการ
- **Defect Photo Annotation** — ถ่ายรูป + ใส่คำอธิบายจุดบกพร่อง
- **PM Check Sheet with Timestamp** — ไม่ใช่แค่ tick ✓ แต่ต้องมีค่าวัดจริง

### 25.2.5 Mini-Project ท้ายฝึกงาน (แนะนำเพิ่ม)

เลือกแม่พิมพ์ 1 ตัว แล้วทำครบวงจร:
1. ทำ PM ครบตามขั้นตอน (ถอด → ทำความสะอาด → วัด → ประกอบ)
2. บันทึก digital record ด้วย Python (shot count, part replaced, dimension)
3. สร้าง PM Dashboard อย่างง่าย (matplotlib)
4. เขียน Maintenance Report ฉบับสมบูรณ์

→ ได้ทั้ง **portfolio** สำหรับตัวเอง และ **deliverable** จริงให้บริษัท

---

## 25.3 Open-Source Tools เพิ่มเติมสำหรับ Mold & Die

เพิ่มเข้า Section 17.2 Catalog:

### M. Mold & Die Engineering (ใหม่ — เพิ่มจากแผนฝึกงาน Toshiba)

| Capability | OSS Tool | License | สถานะ | หมายเหตุ |
|---|---|---|---|---|
| อ่าน/view 3D mold drawing | **FreeCAD** | LGPL-2.0 | ✅ แนะนำ | รองรับ STEP/IGES/DXF — format มาตรฐานอุตสาหกรรม |
| FEA วิเคราะห์ stress (Punch/Die) | **CalculiX** | GPL-2.0 | 🔄 mini-project | วิเคราะห์ stress concentration บน punch corner |
| Pre/Post FEA mesh | **Salome Platform** | LGPL-2.1 | 🔄 mini-project | ใช้คู่กับ CalculiX |
| วาด/แก้ 2D Drawing (DXF) | **LibreCAD** | GPLv2 | ✅ แนะนำ | อ่านและสร้าง .dxf ได้โดยตรง |
| Parse DXF → extract dimension | **ezdxf** (Python) | MIT | ✅ ใช้ได้เลย | ดึง dimension, tolerance จาก .dxf อัตโนมัติ |
| Mold defect detection (custom) | **YOLOv8 fine-tune** | AGPL-3.0 | 🔄 Phase 3+ | train บน defect ของโรงงานจริง (Flash, Burr, Sink) |
| Defect annotation บนรูป | **Label Studio** | Apache 2.0 | ✅ ใช้ label dataset | self-hosted, export YOLO format ได้ |
| PM Dashboard | **Grafana + SQLite** | AGPL-3.0 | 🔄 Phase 4 | plot shot count, downtime, part life แบบ real-time |
| QR/Barcode สำหรับ mold tracking | **qrcode** + **python-barcode** | MIT/MIT | ✅ ใช้ได้เลย | print QR ติด mold → scan → pull PM history |
| Measurement data compare vs drawing | **pandas + sympy + pint** | BSD/BSD/BSD | ✅ ใช้ library เดิม | เทียบ measured value กับ nominal ± tolerance |
| Cooling channel CFD (ขั้นสูง) | **OpenFOAM** | GPL-3.0 | ❌ Defer ออก | เกิน scope ฝึกงาน — setup ใช้เวลา 3-7 วัน (ดู G-15) |
| Material property lookup | **mpmath** + custom DB | BSD | ✅ สร้าง DB เอง | เก็บ hardness, Tg, yield strength ของ SKD11, P20 ฯลฯ |

### N. Mold Defect Photo Workflow (ใหม่)

| Capability | OSS Tool | License | สถานะ |
|---|---|---|---|
| Annotate defect บนรูปถ่าย | **GIMP** (batch script) | GPL-3.0 | ✅ ใช้ได้เลย |
| Vector annotation / ลูกศรชี้จุด | **Inkscape** | GPL-3.0 | ✅ ใช้ได้เลย |
| Image diff ก่อน/หลังซ่อม | **opencv-python** | Apache 2.0 | ✅ library เดิม |
| Compress + archive รูปแม่พิมพ์ | **Pillow** | HPND | ✅ library เดิม |

---

## 25.4 ปรับ Knowledge Domain ใน Section 6

ควรเพิ่ม document type เหล่านี้เข้าไปใน **Layer 1 (Wiki)**:

```yaml
mold_die_knowledge_base:
  technical_docs:
    - "Mold & Die Component Manual (ภาษาไทย/ญี่ปุ่น/อังกฤษ)"
    - "Material Data Sheet: SKD11, SKD61, S50C, P20, NAK80"
    - "Injection Molding Process Parameter Guide"
    - "Press Die Design Standard (Clearance, Radius, Springback)"
    - "Plastic Material Property: PP, ABS, PC (สำหรับ washing machine parts)"

  internal_records:
    - "PM Check Sheet ประวัติทุก mold (Excel)"
    - "Defect Report + Photo Archive"
    - "Drawing เปรียบกับชิ้นงานจริง (measurement log)"
    - "Shot counter log ต่อแม่พิมพ์"

  safety_docs:
    - "SOP การยกแม่พิมพ์ (crane + rigging)"
    - "SOP การถอดประกอบแม่พิมพ์"
    - "5S Checklist พื้นที่ทำงาน"
    - "MSDS สารหล่อลื่น / น้ำยาทำความสะอาด"
```

---

## 25.5 ปรับ LangGraph Workflow — เพิ่ม Mold Analyse Node

```python
# เพิ่มใน Section 10 (Pipeline หลัก)
# Node ใหม่ที่ควรสร้างใน Phase 3

class MoldAnalyseNode:
    """
    รับ input: defect description + process log + drawing
    ส่ง output: root cause candidates + recommended action
    """
    
    def analyse(self, state: HandOffPacket) -> HandOffPacket:
        defect = state.defect_type          # e.g. "Flash"
        process_log = state.process_params  # temp, pressure, cycle time
        
        # Step 1: Match defect กับ cause matrix
        cause_matrix = self._load_defect_cause_matrix(defect)
        
        # Step 2: ตรวจ process param เทียบ spec
        param_deviations = self._check_process_params(process_log)
        
        # Step 3: Cross-reference กับ PM history ของแม่พิมพ์นี้
        pm_history = self._query_pm_history(state.mold_id)
        
        # Step 4: LLM synthesize root cause (ผ่าน Ollama local)
        root_cause = self.llm.analyse(
            defect=defect,
            deviations=param_deviations,
            pm_history=pm_history,
            cause_matrix=cause_matrix
        )
        
        return state.update(root_cause=root_cause)
```

---

## 25.6 ปรับ Implementation Phases — เพิ่ม Mold Domain Tasks

เพิ่มใน **Phase 1 (Knowledge Layer)**:
- [ ] สร้าง Material DB (SKD11/61, S50C, P20, NAK80) ด้วย pandas
- [ ] Import PM Check Sheet ประวัติแม่พิมพ์เข้า ChromaDB
- [ ] Setup FreeCAD + ezdxf สำหรับอ่าน drawing

เพิ่มใน **Phase 3 (Workers + Tools)**:
- [ ] สร้าง `MoldAnalyseNode` ใน LangGraph
- [ ] สร้าง Mold PM Tracker (shot counter + part life alert)
- [ ] สร้าง Defect Catalogue สำหรับ Injection + Press Die
- [ ] QR code generator สำหรับ mold ID tracking

เพิ่มใน **Phase 4 (UI + Dashboard)**:
- [ ] PM Dashboard (Grafana หรือ matplotlib) — shot count, downtime per mold
- [ ] Defect trend chart — defect type ต่อเดือน
- [ ] Measurement log viewer — วัดได้ vs tolerance ± จาก drawing

---

## 25.7 Risk Register เพิ่มเติม (Mold Domain)

| ID | Risk | ความน่าจะเป็น | ผลกระทบ | Mitigation |
|---|---|---|---|---|
| R-M1 | Drawing format ที่บริษัทใช้ไม่ใช่ DXF/STEP (proprietary) | 🟡 Medium | 🟠 High | ถ่ายภาพ + OCR เป็น fallback; ขอ export STEP จาก CAD ต้นทาง |
| R-M2 | Process parameter อยู่ใน PLC/Machine — เข้าถึงไม่ได้ | 🟡 Medium | 🟡 Medium | บันทึกด้วยมือ (manual log) → import Excel → ระบบ |
| R-M3 | YOLO defect model accuracy ต่ำ (ขาด training data) | 🟠 High | 🟡 Medium | เริ่มจาก rule-based + keyword matching → สะสม data → train ทีหลัง |
| R-M4 | ไม่ได้รับอนุญาตถ่ายภาพแม่พิมพ์ (confidentiality) | 🟡 Medium | 🟡 Medium | ใช้ sketch / schematic แทนรูปถ่ายจริง |
| R-M5 | Shot counter ไม่มีในระบบเดิม → ต้องเริ่มนับใหม่ | 🟠 High | 🟢 Low | Reset counter ณ วันเริ่มระบบ + ประมาณจาก PM history ย้อนหลัง |

---

> **สรุป:** MASTER_PLAN_v5.0 ครอบคลุม infrastructure ดี (LLM, RAG, pipeline, UI) แต่ขาด
> **domain knowledge เฉพาะ Mold & Die** และ **process parameter awareness**
> Section นี้เติมช่องว่างนั้น — ใช้ควบคู่กับแผนฝึกงาน Toshiba ได้ทันที


---

# 26. Gap Analysis & Patch Notes — ตรวจสอบรอบที่ 1 (9 พฤษภาคม 2026)

> ตรวจสอบแผนทั้งหมดแบบ end-to-end พบช่องโหว่ **15 จุด** แบ่งเป็น Critical 5 / High 5 / Medium 5
> ทุกจุดมีแนวทางแก้ไขระบุไว้ — ควรอ่านก่อนเริ่ม Phase 0

---

## 26.1 🔴 CRITICAL — ต้องแก้ก่อนเริ่ม Phase ใดทั้งสิ้น

---

### G-01 — OpenKB และ PageIndex อาจไม่มีอยู่จริงบน PyPI

**จุดที่มีปัญหา:** Section 6.2, Phase 1 Tasks
```bash
pip install openkb    # ← ไม่ปรากฏใน PyPI index ที่รู้จัก
pip install pageindex # ← ไม่ปรากฏใน PyPI index ที่รู้จัก
```
**ความเสี่ยง:** Layer 1 (Local Knowledge) ทั้งหมดพัง — Phase 1 ล้มตั้งแต่วันแรก เป็นจุดที่ส่งผลกระทบต่อทุก Phase ที่ตามมา

**แนวทางแก้ไข:**
```yaml
ตรวจสอบก่อน Phase 1:
  step_1: pip search openkb && pip search pageindex  # ดูว่ามีจริงไหม
  step_2: ถ้าไม่มี → เปลี่ยนไปใช้ stack ที่ verified แทน:

stack_ทดแทน_Layer1:
  document_parsing:  "markitdown (Microsoft, MIT) — แปลง PDF/Word/Excel → Markdown"
  vector_index:      "llama-index (MIT) — document ingestion + query"
  wiki_viewer:       "Obsidian vault (markdown ธรรมดา) — ดูได้โดยไม่ต้อง compile"
  semantic_search:   "ChromaDB + bge-m3 (ใช้อยู่แล้ว)"

คำสั่งแทน:
  pip install markitdown llama-index llama-index-embeddings-ollama
```
**Priority:** แก้ก่อน Phase 1 เริ่ม — ทดสอบ install ได้วัน Phase 0 เลย

---

### G-02 — VRAM Vision Conflict ยังไม่มีคำตอบชัดเจน

**จุดที่มีปัญหา:** Section 4.4 (ตาราง Vision Task)
```
❌ เกิน 8 GB!
...
```
**ความเสี่ยง:** มี 3 options แต่ไม่มี default — เมื่อถึง Phase 3 จะไม่รู้จะทำอะไร

**แนวทางแก้ไข — กำหนด default ทันที:**
```yaml
vision_default_strategy: "Option C ก่อน"

option_C_first:
  model: "qwen2.5-vl:3b-q4_K_M  (หรือ qwen3-vl:2b ถ้ามี)"
  VRAM: "~2 GB → รวม orchestrator+scavenger = ~8 GB พอดี"
  trade_off: "accuracy ต่ำกว่า 7B ~15% — ยอมรับได้สำหรับงาน nameplate/defect photo"

fallback_to_B_ถ้า_C_ไม่พอ:
  strategy: "CPU+GPU split สำหรับ 7B model"
  command: "ollama run qwen2.5-vl:7b --num-gpu-layers 20"
  speed: "~5-8 tok/s (ช้ากว่า 3x แต่ accuracy ดีกว่า)"

ห้ามใช้_option_A:
  reason: "Unload orchestrator ทำให้ระบบ stateless — LangGraph checkpoint อาจสูญ"
```

---

### G-03 — Docker Desktop ไม่อยู่ใน Phase 0 Setup

**จุดที่มีปัญหา:** Phase 0 Tasks ไม่มีขั้นตอนติดตั้ง Docker แต่ Phase 1 ต้องการ:
- SearXNG: `docker compose up -d searxng redis`
- Langfuse: Docker
- Arize Phoenix: Docker

**ความเสี่ยง:** Phase 1 day 4 จะหยุดทันที — setup Docker บน Windows 11 + WSL2 ใช้เวลา 30-90 นาที และมี pitfalls (Hyper-V conflict, WSL2 integration)

**แนวทางแก้ไข — เพิ่มเข้า Phase 0 day_1_morning:**
```yaml
phase_0_เพิ่ม_docker:
  - [ ] ติดตั้ง Docker Desktop for Windows (จาก docker.com)
  - [ ] Enable WSL2 backend ใน Docker Desktop settings
  - [ ] ทดสอบ: docker run hello-world  # ต้องผ่านก่อนไปต่อ
  - [ ] docker compose version  # ต้องได้ v2.x ขึ้นไป
  - [ ] ทดสอบ GPU passthrough: docker run --gpus all nvidia/cuda:12.0-base nvidia-smi

acceptance_criteria_phase0_เพิ่ม:
  - docker run hello-world ผ่าน
  - nvidia-smi ใน docker container เห็น RTX 5060
```

---

### G-04 — Night Cycle จะดับเมื่อ Windows Sleep

**จุดที่มีปัญหา:** Section 15 Night Cycle รัน 23:00-05:00 แต่ไม่มีการป้องกัน Windows Sleep/Hibernate

**ความเสี่ยง:** Night Cycle ไม่เคย complete ได้จริง — เครื่องส่วนใหญ่ sleep หลัง 30 นาทีไม่มี activity

**แนวทางแก้ไข — เพิ่ม script ใน Phase 0:**
```powershell
# scripts/prevent_sleep.ps1 — รันก่อน night cycle เริ่ม
# ป้องกัน sleep ตลอด night window

# วิธี 1: PowerShell (ง่ายสุด)
powercfg /change standby-timeout-ac 0       # ไม่ sleep เมื่อเสียบปลั๊ก
powercfg /change monitor-timeout-ac 0       # จอดับได้ แต่เครื่องไม่ sleep

# วิธี 2: Windows Task Scheduler (ดีกว่า — auto)
# สร้าง task: ทุกวัน 22:55 → รัน prevent_sleep.ps1
# สร้าง task: ทุกวัน 05:05 → คืน sleep settings ปกติ

# วิธี 3: ใน start.sh / night_cycle.py
import ctypes
...
```
```yaml
phase_0_เพิ่ม_sleep_guard:
  - [ ] สร้าง scripts/prevent_sleep.ps1
  - [ ] ทดสอบ: เปิดทิ้งไว้ 1 ชม. โดยไม่แตะ → เครื่องไม่ sleep
```

---

### G-05 — GitHub Backup ไม่รองรับ Models Folder (~80 GB)

**จุดที่มีปัญหา:** Section 20 GitHub Backup Strategy กำหนด backup ทั้ง workspace แต่:
- GitHub file limit: 100 MB ต่อไฟล์
- GitHub repo recommended limit: 1 GB
- models/ folder: ~80 GB (8 models × ~5-22 GB)

**ความเสี่ยง:** `git push` จะ fail ถ้า models/ ไม่ถูก exclude — หรือ repo พอง 80 GB แล้ว GitHub block

**แนวทางแก้ไข:**
```gitignore
# .gitignore ต้องมีชัดเจน (เพิ่มใน Phase 0)
models/           # ~80 GB GGUF files — ไม่ backup บน GitHub
chroma_db/        # vector index — rebuild ได้
__pycache__/
*.pyc
.venv/
logs/             # rotate แล้ว ไม่จำเป็น
```
```yaml
model_backup_strategy:
  github: "เฉพาะ code + config + wiki markdown + prompts"
  models_folder: "ไม่ backup — download ใหม่จาก HuggingFace/Ollama ได้ตลอด"
  model_list_file: "บันทึกแค่ชื่อ model ใน models/MODEL_LIST.txt → push ไป GitHub"
  restore_procedure: "ดู MODEL_LIST.txt → รัน scripts/download_models.sh"

scripts/download_models.sh_ตัวอย่าง:
  - ollama pull qwen3:8b-instruct-q4_K_M
  - ollama pull qwen3:1.7b-instruct-q4_K_M
  - ollama pull bge-m3
  - "# (รายการจาก MODEL_LIST.txt)"
```

---

## 26.2 🟠 HIGH — แก้ก่อน Phase ที่เกี่ยวข้องเริ่ม

---

### G-06 — ไม่มี Version Pinning ใน Dependencies

**จุดที่มีปัญหา:** Phase 0 ติดตั้งโดยไม่ lock version:
```bash
pip install langgraph langchain-core pydantic  # ← ไม่มี version
```
**ความเสี่ยง:** Breaking change จาก upstream ทำให้ระบบพัง โดยเฉพาะ LangGraph ที่ยัง active development

**แนวทางแก้ไข — สร้าง requirements.txt ก่อน Phase 0:**
```
# requirements.txt (pin major.minor อย่างน้อย)
langgraph>=0.2,<0.3
...
ollama>=0.3,<0.4
chainlit>=1.3,<2.0
markitdown>=0.1        # ทดแทน openkb (ดู G-01)
ruff>=0.4
bandit>=1.7
pytest>=8.0
```
```bash
# Phase 0 day 1: ใช้ requirements.txt เสมอ
pip install -r requirements.txt
pip freeze > requirements.lock  # lock exact version สำหรับ reproduce
```

---

### G-07 — Section 25 (Mold & Die) ไม่ถูก Integrate เข้า Acceptance Criteria

**จุดที่มีปัญหา:** Section 25 เพิ่ม tasks เข้า Phase 1, 3, 4 แต่:
- Acceptance Criteria ของแต่ละ Phase ไม่ได้อัปเดต
- golden_dataset ไม่มี mold-specific test cases เลย
- ไม่มี definition ว่า MoldAnalyseNode "ผ่าน" เมื่อไหร่

**แนวทางแก้ไข — เพิ่ม mold test cases เข้า golden_dataset:**
```yaml
golden_dataset_mold_cases_เพิ่ม:
  case_M1:
    input: "แม่พิมพ์ #A12 มี Flash ที่ parting line ทุก shot"
    expected_output: "root cause: clamping force ต่ำ / parting surface สึก + recommended action"
    must_include: ["clamping", "parting line", "PM history"]

  case_M2:
    input: "ชิ้นงาน PP มี Sink mark ที่ผนังหนา 4 mm"
    expected_output: "เพิ่ม holding pressure / ลด gate size / ตรวจ cooling time"

  case_M3:
    input: "Punch SKD11 ผ่านมา 120,000 strokes ควรทำ PM ไหม"
    expected_output: "เกิน interval 100,000 strokes → ถึงเวลา PM + รายการตรวจสอบ"

  case_M4:
    input: "อ่าน PM check sheet แม่พิมพ์ B7 แล้วสรุปสถานะ"
    expected_output: "สรุป: ซ่อมครั้งล่าสุด, part ที่เปลี่ยน, shot count ปัจจุบัน"

acceptance_criteria_phase3_เพิ่ม:
  - MoldAnalyseNode ตอบ case_M1 ได้ถูกต้อง (root cause match)
  - Shot counter alert ทำงานเมื่อ shots เกิน threshold
  - golden_dataset mold cases ≥ 3/4 ผ่าน
```

---

### G-08 — active-win-listener บน WSL2 ทำงานไม่ได้จริง

**จุดที่มีปัญหา:** Section 14 Activity Tracking Tier 3 ใช้ `active-win-listener` และ `xdotool` ผ่าน WSL2

**ความจริง:**
- `xdotool` ต้องการ X11 display — WSL2 ไม่มี X11 โดย default (ต้องใช้ WSLg หรือ VcXsrv)
- `active-win-listener` (Node.js) ทำงานบน native OS ไม่ใช่ WSL2 — จะเห็นแค่ WSL2 windows ไม่เห็น Windows apps

**แนวทางแก้ไข:**
```yaml
windows_tracking_ทดแทน:
  วิธี_1_PowerShell_script:
    tool: "Get-Process | Where-Object {$_.MainWindowTitle}"
    ข้อดี: "native Windows, ไม่ต้องติดตั้งอะไรเพิ่ม"
    วิธีเรียก: "subprocess จาก Python ใน WSL2 ผ่าน powershell.exe"

  วิธี_2_Python_native_Windows:
    tool: "pygetwindow + win32gui (Python บน Windows native ไม่ใช่ WSL)"
    ข้อดี: "เห็น Windows foreground app ได้โดยตรง"
    ข้อเสีย: "ต้อง run tracking script บน Windows Python แยกจาก WSL2"

  ข้อแนะนำ: "ลด scope Tier 3 ลง — tracking outside AI ไม่ใช่ core feature"
  priority: "defer ไป Phase 5 หรือ optional"
```

---

### G-09 — Mem0 Self-Hosted Dependencies ไม่ระบุ

**จุดที่มีปัญหา:** Section 8 (Inquiry-First) ใช้ `Mem0` self-hosted แต่ไม่มี setup steps

**ความจริง:** Mem0 self-hosted ต้องการ:
- Qdrant (vector store แยก) หรือ
- PostgreSQL + pgvector extension

**แนวทางแก้ไข:**
```bash
# เพิ่มใน Phase 2 setup (ก่อน Inquiry-First node)
# Option A: Qdrant (แนะนำ — เร็วกว่า)
docker run -p 6333:6333 qdrant/qdrant

# Option B: ใช้ ChromaDB ที่มีอยู่แล้วแทน Mem0
# ง่ายกว่า ไม่ต้อง service ใหม่ — เก็บ user-specific answers ใน ChromaDB collection "user_memory"
# trade-off: ไม่มี memory graph เหมือน Mem0 แต่พอใช้งานได้

mem0_ทดแทน_ง่ายกว่า:
  tool: "ChromaDB collection 'user_memory'"
  schema: "{question_hash, answer, timestamp, confirmed_by_user}"
  ข้อดี: "ใช้ ChromaDB ที่มีอยู่ — ไม่ต้อง service ใหม่"
```

---

### G-10 — SearXNG Self-Hosted ถูก Block โดย Search Engines

**จุดที่มีปัญหา:** Section 6.4.1 อธิบาย SearXNG ว่า "FREE 100% ไม่จำกัด query" แต่ในทางปฏิบัติ:

- Google block IP ของ SearXNG instance ที่ query บ่อยภายใน 1-3 วัน
- Bing มี rate limit และ CAPTCHA
- ผลลัพธ์จะเหลือแค่ DuckDuckGo + engine เล็กๆ

**แนวทางแก้ไข:**
```yaml
searxng_rate_limit_mitigation:
  config: "settings.yml — จำกัด engines ที่ใช้และ request interval"
  engines_ปลอดภัย:
    - DuckDuckGo     # ไม่ block aggressive
    - Brave          # ค่อนข้างเสถียร
    - Startpage      # Google proxy — เสถียรกว่า Google direct
    - Qwant          # European, ไม่ strict
  
  ปิด_engines_เสี่ยง:
    - Google (direct)   # block เร็วมาก
    - Bing (direct)     # rate limit เข้มงวด

  request_interval: "≥ 3 วินาที ระหว่าง query เพื่อหลีกเลี่ยงการถูก block"
  
  ถ้า_SearXNG_พัง:
    fallback_1: "DuckDuckGo HTML scrape (trafilatura)"
    fallback_2: "Tavily API (1,000 free/mo)"
    ไม่มี_workaround: "บันทึกใน Risk Register R7"
```

---

## 26.3 🟡 MEDIUM — แก้เมื่อถึง Phase ที่เกี่ยวข้อง

---

### G-11 — Qwen3-Coder ชื่อ Model อาจไม่ตรงกับ Ollama Registry

**จุดที่มีปัญหา:** Phase 3 Tasks:
```bash
ollama pull qwen3-coder:8b-q4_K_M  # ← ชื่อนี้อาจไม่มีใน Ollama registry
```
**แนวทางแก้ไข:**
```bash
# ก่อน Phase 3: ตรวจสอบก่อน
ollama search qwen3-coder
ollama search qwen2.5-coder

# ถ้าไม่มี qwen3-coder → ใช้:
ollama pull qwen2.5-coder:7b-q4_K_M  # มีใน registry แน่นอน (verified)
# หรือ
ollama pull deepseek-coder-v2:16b-lite-instruct-q4_K_M  # alternative ที่ดี
```

---

### G-12 — ไม่มี Backup Restore Drill

**จุดที่มีปัญหา:** Section 20 มี backup strategy แต่ไม่มีขั้นตอนทดสอบว่า restore ได้จริง

**แนวทางแก้ไข — เพิ่มใน Phase 0 acceptance criteria:**
```yaml
restore_drill_phase0:
  - [ ] สร้าง test file → commit → push → ลบออก → pull → ไฟล์กลับมา
  - [ ] บันทึกขั้นตอน restore ใน RUNBOOK.md
  - [ ] ทดสอบ restart LangGraph จาก checkpoint: หยุดกลางทาง → resume → ผลเหมือนกัน

ทำซ้ำ: ทุกเดือน (ใส่ใน night cycle monthly task)
```

---

### G-13 — Golden Dataset เล็กเกินไปสำหรับ Production Validation

**จุดที่มีปัญหา:**
- Phase 2: ผ่านถ้า 5/10 cases ✓ (50% pass rate!)
- Phase 3: ผ่านถ้า 8/10 cases ✓
- 10 cases น้อยเกินไปสำหรับการ validate ระบบจริง

**แนวทางแก้ไข:**
```yaml
golden_dataset_expansion:
  phase_2_target: "≥ 20 cases (เพิ่มจาก 10)"
  phase_3_target: "≥ 30 cases"
  phase_5_target: "≥ 50 cases"
  
  pass_rate_ที่ควรเป็น:
    phase_2: "12/20 (60%) — ยังช่วงพัฒนา"
    phase_3: "24/30 (80%)"
    phase_5: "48/50 (96%)"

  วิธีเพิ่ม_cases_เร็ว:
    - "เก็บจาก real usage: ทุก query ที่ approve → เข้า golden_dataset อัตโนมัติ"
    - "Night cycle สร้าง synthetic cases จาก pattern ที่พบ"
```

---

### G-14 — Windows/WSL2 Path Inconsistency ทั่วทั้งแผน

**จุดที่มีปัญหา:** แผนผสม path แบบ Windows และ Linux:
```
~/tpm_workspace/                    ← Linux path
/mnt/d/tpm_workspace               ← WSL2 path ของ Windows D:\
D:\tpm_workspace                    ← Windows native path
```
ไม่มีการระบุชัดว่า code รันใน environment ไหน

**แนวทางแก้ไข — กำหนด canonical path:**
```yaml
canonical_paths:
  workspace_windows: "D:\\tpm_workspace"
  workspace_wsl2:    "/mnt/d/tpm_workspace"
  workspace_linux:   "/mnt/d/tpm_workspace"  # same as WSL2

run_environment:
  LLM_serving:  "Ollama → Windows native (GPU access ดีกว่า WSL2)"
  Python_code:  "WSL2 Ubuntu 24 (Linux libraries สมบูรณ์กว่า)"
  Docker:       "Docker Desktop → WSL2 backend"
  File_access:  "WSL2 เข้า D:\\ ผ่าน /mnt/d/ — write/read ได้ปกติ แต่ช้ากว่า native ~20%"

กฎ: "ทุก script ใช้ Path variable — ไม่ hardcode path"
```
```python
# ใช้ใน code ทั้งหมด
from pathlib import Path
WORKSPACE = Path("/mnt/d/tpm_workspace")  # WSL2 canonical
```

---

### G-15 — OpenFOAM ใน Section 25 เกินขอบเขตฝึกงาน

**จุดที่มีปัญหา:** Section 25.3 แนะนำ OpenFOAM (CFD) สำหรับวิเคราะห์ cooling channel

**ความจริง:**
- OpenFOAM setup ใช้เวลา 3-7 วันสำหรับมือใหม่
- ต้องการพื้นฐาน CFD และ meshing (Salome)
- ไม่เหมาะสำหรับช่วงฝึกงาน 2-3 เดือน

**แนวทางแก้ไข:**
```yaml
section_25_patch:
  OpenFOAM: "เปลี่ยน status จาก '🔄 optional/Phase 5' → '❌ defer ออกไปก่อน'"
  เหตุผล: "learning curve สูงเกินไป ไม่คุ้มค่าใน timeline ฝึกงาน"
  
  ทดแทนด้วย:
    tool: "MoldFlow Adviser (ถ้าบริษัทมี) หรือ Solidworks Plastics"
    สำหรับ_intern: "เรียนรู้แค่การอ่าน MoldFlow result — ไม่ต้อง run simulation เอง"
    open_source_จริงๆ: "CalculiX (FEA structural) สมเหตุสมผลกว่า — เรียนได้ใน 1-2 วัน"
```

---

## 26.4 สรุปสถานะหลัง Gap Analysis

| # | Gap | ระดับ | Phase ที่กระทบ | สถานะ |
|---|---|---|---|---|
| G-01 | OpenKB/PageIndex ไม่มีจริง | 🔴 Critical | Phase 1 | ⏳ รอแก้ก่อน Phase 1 |
| G-02 | VRAM Vision ไม่มีคำตอบ | 🔴 Critical | Phase 3 | ⏳ รอ confirm option C |
| G-03 | Docker ไม่อยู่ใน Phase 0 | 🔴 Critical | Phase 0 | ⏳ เพิ่มทันที |
| G-04 | Night Cycle vs Windows Sleep | 🔴 Critical | Phase 4 | ⏳ เพิ่ม prevent_sleep.ps1 |
| G-05 | GitHub 80 GB Models | 🔴 Critical | Phase 0 | ⏳ อัปเดต .gitignore |
| G-06 | ไม่มี Version Pinning | 🟠 High | Phase 0 | ⏳ สร้าง requirements.txt |
| G-07 | Section 25 ไม่ integrate Phase | 🟠 High | Phase 1,3,4 | ⏳ เพิ่ม golden_dataset |
| G-08 | active-win-listener WSL2 พัง | 🟠 High | Phase 4 | ⏳ เปลี่ยน approach |
| G-09 | Mem0 dependencies ไม่ระบุ | 🟠 High | Phase 2 | ⏳ ใช้ ChromaDB แทน |
| G-10 | SearXNG ถูก block | 🟠 High | Phase 1 | ⏳ จำกัด engines |
| G-11 | Qwen3-Coder ชื่อผิด | 🟡 Medium | Phase 3 | ⏳ verify ก่อน pull |
| G-12 | ไม่มี Restore Drill | 🟡 Medium | Phase 0 | ⏳ เพิ่มใน Phase 0 |
| G-13 | Golden Dataset เล็กเกิน | 🟡 Medium | Phase 2,3 | ⏳ ขยายเป็น 20-30 cases |
| G-14 | Windows/WSL2 Path ปน | 🟡 Medium | ทุก Phase | ⏳ กำหนด canonical path |
| G-15 | OpenFOAM เกิน scope | 🟡 Medium | Section 25 | ⏳ defer ออก |

> **คำแนะนำ:** แก้ G-01 ถึง G-05 ให้เสร็จก่อนแตะ Phase 0 day 1
> ถ้าข้ามไปทำ Phase 1 โดยยัง G-01 ค้างอยู่ — Phase 1 จะล้มทั้งหมด
