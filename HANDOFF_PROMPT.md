...
- ⏸️ **edocr2 heavy deps** (item C) — `pip install -r external/edocr2-main/requirements.txt` started in background late-session; status uncertain at handoff (0/7 deps in venv at last check). If still running, let it finish; otherwise re-run the command. Once complete, edocr2 OCR is invocable end-to-end.
