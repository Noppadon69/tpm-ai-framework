...
- ✅ **GitHub push complete** — worktree commits pushed to remote at branch `claude/affectionate-hopper-62e5ac`. [PR #1](https://github.com/Noppadon69/tpm-ai-framework/pull/1) opened on framework repo; private-repo deltas pushed to main.
...
**Autonomous continuation block 2 (Bug #6 permanent + alpha + delta):**
- ✅ **Bug #6 permanent defense** (commit 9432b10) — `tpm_core/clarification.py::parse_intent()` now wraps the LLM call in try/except; on ANY failure (timeout / RuntimeError / ValueError after retries) it falls back to `_fallback_intent_from_rules()` which mirrors the disambiguation rules of `INTENT_PARSER_SYSTEM`. Confidence capped at <= 0.55 so the clarification loop refines it - same recovery path as any other low-confidence LLM result. `parse_intent` now NEVER crashes even if Ollama spins indefinitely. 29 assertions in `tests/test_bug6_fallback.py`.
- ✅ **Vibration worker scaffold** (commit 3331f48, item alpha) — `tpm_workers/vibration.py` matches the existing WorkerInput/Result contract. Pipeline: validate inputs -> load accel CSV -> raw FFT + Hilbert envelope -> harmonic-aware verdict (BPFO/BPFI/BSF/FTF; matches up to 3x with small confidence penalty) -> .md audit. Bearing presets 6204/6205/6206 + custom dict. 20 assertions in `tests/test_vibration_worker.py`. NOT yet wired into orchestrator dispatch - wire when first real Toshiba accel trace arrives.
- ✅ **Mold calc demo** (same commit 3331f48, item delta) — `scripts/demo_mold_calc.py` exercises existing FORMULA_LIBRARY on 4 Toshiba scenarios: clamping force 1.20 MN (200x150 mm PP at 40 MPa), shot weight 109 g (PP/120 cc), cooling time 18 s (3 mm wall), punch corner stress 2000 MPa (over SKD11 yield - flagged). Writes audit to `output/demo/mold_calc.md`.
- ✅ **edocr2 heavy deps installed** (background, autonomous) — 6/7 heavy ML deps now in venv: tensorflow, tf-keras, sentence-transformers, pdf2image, imgaug, efficientnet. paddleocr not listed in edocr2's requirements so skipped. edocr2 OCR is now end-to-end runnable when first real drawing PDF arrives. Day-1 step is now just `python -c "from edocr2.tools import ocr_pipelines"`.

**Regression checked (main checkout):** all 10 original offline test suites PASS (~280 assertions, 0 fail). New worktree-only suites: test_bug6_fallback 29/29, test_vibration_worker 20/20. Combined target after merge: ~330 assertions across 12 suites.

**Autonomous continuation block 3 (Telegram bot live):**
- ✅ **Bot token wired** — user pasted token; saved to `.env` (gitignored). Token verified with getMe -> bot username `@TPMNantan_bot`, id 8831665956. Owner's chat_id (7057665544) added to `TPM_TELEGRAM_ALLOWED_USERS`.
- ✅ **Polling bridge live** (commits 4adacc6 + 7c872e7 + 30d2d01) — `scripts/telegram_bridge.py`. Pipeline: long-poll getUpdates -> dispatch -> sendMessage.
- ✅ **truststore baked in** — Avast/MITM cert intercept on this Windows laptop was failing `api.telegram.org` cert verify; `truststore.inject_into_ssl()` makes Python use Windows cert store and the intermediates resolve. `truststore>=0.10` added to requirements.txt.
- ✅ **Allowlist gate** — `TPM_TELEGRAM_ALLOWED_USERS=` comma list in .env. Empty = `/me` + `/help` only (so owner can discover their id), otherwise plain text + commands flow through. `/me` always returns chat_id regardless of auth.
- ✅ **Commands (all fast, no LLM)** — `/me` `/help` `/status` `/defect <name>` `/pm <mold> [sub]` `/calc <expr>`. Registered with Telegram via `setMyCommands` -> autocomplete in client.
- ✅ **Photo handler** — message with photo -> pick biggest size -> getFile + HTTPS download to `raw_data/telegram_photos/` -> `scripts/analyze_image.py <local> [--prompt <caption>]` -> reply with top 30 lines of analyzer output. Day-1 flow: operator photographs Flash defect on the line -> sends to bot -> gets defect ID + suggested actions in chat.
- ✅ **36 assertions** in `tests/test_telegram_bridge.py` (allowlist parse, dispatch routing, auth gate, photo path mocked end-to-end).
- ⚠️ **Bridge process is foreground-bound to whichever shell launched it** — when Claude session ends or system reboots, you need to re-run `python scripts/telegram_bridge.py`. If you want it always-on, wire it into Windows Task Scheduler "at logon" or wrap with `nssm` as a service.
- ⚠️ **Token in transcript** — the token was pasted in chat so it lives in the Anthropic API log forever. Recommend `/revoke` at @BotFather after this is done with, then update `.env`.
